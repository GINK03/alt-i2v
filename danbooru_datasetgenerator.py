...
if __name__ == '__main__':
  ...
  if mode == 'scrape':
    ...
    def analyzing(inputs) -> str:
      ...
      aside_li = soup.find('aside', {'id':'sidebar'}).find_all('li')
      rating = list(filter(lambda x:'Rating' in x, [li.text for li in aside_li]))[0].replace('Rating: ', '')
      copyright = aside_li[0].find_all('a')[1].text
      print(copyright)
      print(rating)
      meta = {'copyright':copyright, 'rating': rating}
      ...
      def _i(img_url, data_tags, index, meta):
        opener  = urllib.request.build_opener()
        request = urllib.request.Request(url=img_url)
        con = opener.open(request).read()
        open('./danbooru.imgs/{}.jpg'.format( img_url.split('/')[-1] ), 'wb').write(con)
        open('./danbooru.imgs/{name}.txt'.format(name=img_url.split('/')[-1] ), 'w').write(data_tags)
        open('./danbooru.imgs/{name}.metav1'.format(name=img_url.split('/')[-1] ), 'w').write(json.dumps(meta))
        open('./danbooru.finished/{index}'.format(index=index), 'w').write("f")
        print('complete storing image of {url}'.format(url=img_url) )
      th = Th(target=_i, args=(img_url, data_tags, index, meta, ))
      th.start()
      return sec.find('img').get('data-tags')
    
    finished = set(name.split('/')[-1] for name in glob.glob('./danbooru.finished/*'))
    samples  = filter( lambda x:x not in finished, range(1, 2653551))
    urls = [ ('http://danbooru.donmai.us/posts/{i}'.format(i=i), i) for i in samples]
    random.shuffle(urls)
